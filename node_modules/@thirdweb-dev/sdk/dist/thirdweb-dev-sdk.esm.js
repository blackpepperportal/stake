import { BigNumber, utils, constants, ethers, providers, Signer, Contract } from 'ethers';
import { z } from 'zod';
import { FileBufferOrStringSchema, IpfsStorage, RemoteStorage } from '@thirdweb-dev/storage';
import { v4 } from 'uuid';
...
import DelayedRevealAbi$1 from '@thirdweb-dev/contracts-js/dist/abis/DelayedReveal.json';
...
import { Forwarder__factory, IERC20__factory, TWRegistry__factory, TWFactory__factory, IThirdwebContract__factory } from '@thirdweb-dev/contracts-js';
...
import ABI$1 from '@thirdweb-dev/contracts-js/dist/abis/DropERC1155.json';
import ABI$2 from '@thirdweb-dev/contracts-js/dist/abis/Marketplace.json';
import ABI$3 from '@thirdweb-dev/contracts-js/dist/abis/Multiwrap.json';
import ABI$4 from '@thirdweb-dev/contracts-js/dist/abis/TokenERC721.json';
import ABI$5 from '@thirdweb-dev/contracts-js/dist/abis/DropERC721.json';
import ABI$6 from '@thirdweb-dev/contracts-js/dist/abis/Pack.json';
import ABI$7 from '@thirdweb-dev/contracts-js/dist/abis/SignatureDrop.json';
import ABI$8 from '@thirdweb-dev/contracts-js/dist/abis/Split.json';
import ABI$9 from '@thirdweb-dev/contracts-js/dist/abis/TokenERC20.json';
import ABI$a from '@thirdweb-dev/contracts-js/dist/abis/DropERC20.json';
import ABI$b from '@thirdweb-dev/contracts-js/dist/abis/VoteERC20.json';
...
const JsonSchema = z.lazy(() => z.union([JsonLiteral, z.array(JsonSchema), z.record(JsonSchema)]));
const JsonObjectSchema = z.record(JsonSchema);
const HexColor = z.union([z.string().regex(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/, "Invalid hex color").transform(val => val.replace("#", "")), z.string().length(0)]);
...
const SnapshotAddressInput = z.object({
...
const SnapshotInputSchema = z.union([z.array(z.string()).transform(strings => strings.map(address => SnapshotAddressInput.parse({
  address
...
const OptionalPropertiesInput = z.union([z.array(JsonObjectSchema), JsonObjectSchema]).optional();

/**
 * @internal
 */

const CommonTokenInput = z.object({
  ...
  constructor(reason, from, to, data, network, rpcUrl, raw, functionInfo) {
    ...
    if (functionInfo) {
      ...
      if (functionInfo.value.gt(0)) {
        ...
        builtErrorMsg += withSpaces("value", "".concat(ethers.utils.formatEther(functionInfo.value), " ").concat((_NATIVE_TOKENS = NATIVE_TOKENS[network.chainId]) === null || _NATIVE_TOKENS === void 0 ? void 0 : _NATIVE_TOKENS.symbol));
      }
    }
...
function hashLeafNode(address, maxClaimableAmount) {
  return utils.solidityKeccak256(["address", "uint256"], [address, BigNumber.from(maxClaimableAmount)]);
}
...
const FEATURE_EDITION_REVEALABLE = {
  ...
  abis: [Erc1155Abi, ILazyMintAbi, DelayedRevealAbi$1],
  features: {}
};
...
function extractIPFSHashFromBytecode(bytecode) {
  ...
  if (cborData["ipfs"]) {
    return "ipfs://".concat(toB58String(cborData["ipfs"]));
  }
...
const BYOCContractMetadataSchema = CommonContractSchema.catchall(z.lazy(() => JsonSchema));
/**
 * @internal
 */
...
async function getPolygonGasPriorityFee(chainId) {
  ...
  try {
    const data = await (await fetch(gasStationUrl)).json(); // take the standard speed here, SDK options will define the extra tip
...
async function signEIP2612Permit(signer, currencyAddress, owner, spender, value, deadline, nonce) {
  ...
  deadline = (_deadline2 = deadline) !== null && _deadline2 !== void 0 ? _deadline2 : ethers.constants.MaxUint256;
  const message = {
    owner,
    spender,
    value,
...
class ContractWrapper extends RPCConnectionHandler {
  ...
  constructor(network, contractAddress, contractAbi, options) {
    ...
    this.writeContract = new Contract(contractAddress, contractAbi, this.getSignerOrProvider()); // setup the read only contract
...
async function setErc20Allowance(contractToApprove, value, currencyAddress, overrides) {
  if (isNativeToken(currencyAddress)) {
    ...
    const erc20 = new ContractWrapper(signer || provider, currencyAddress, ERC20Abi, {});
...
async function updateExistingClaimConditions(index, claimConditionInput, existingConditions) {
  ...
  const priceInTokens = ethers.utils.formatUnits(priceInWei, priceDecimals); // merge existing (output format) with incoming (input format)
...
class Erc20SignatureMintable {
  ...
  async generateBatch(payloadsToSign) {
    ...
    invariant(signer, "No signer available"); // ERC20Permit (EIP-712) spec differs from signature mint 721, 1155.
...
class Erc721Droppable {
  ...
  async lazyMint(metadatas, options) {
    ...
    const receipt = await this.contractWrapper.sendTransaction("lazyMint", [batch.length, baseUri.endsWith("/") ? baseUri : "".concat(baseUri, "/"), ethers.utils.toUtf8Bytes("")]);
...
class Erc721 {
  ...
  async get(tokenId) {
    const [owner, metadata] = await Promise.all([this.ownerOf(tokenId).catch(() => constants.AddressZero), this.getTokenMetadata(tokenId)]);
    return {
      owner,
      metadata
    };
...
class Erc1155Droppable {
  ...
  async lazyMint(metadatas, options) {
    ...
    if (isLegacyEditionDropContract) {
      ...
      receipt = await this.contractWrapper.sendTransaction("lazyMint", [batch.length, "".concat(baseUri.endsWith("/") ? baseUri : "".concat(baseUri, "/")), ethers.utils.toUtf8Bytes("")]);
    }
...
class Erc1155 {
  ...
  async get(tokenId) {
    const [supply, metadata] = await Promise.all([this.contractWrapper.readContract.totalSupply(tokenId).catch(() => BigNumber.from(0)), this.getTokenMetadata(tokenId)]);
...
async function handleTokenApproval(signerOrProvider, marketplaceAddress, assetContract, tokenId, from) {
  const erc165 = new Contract(assetContract, ERC165Abi, signerOrProvider);
  const isERC721 = await erc165.supportsInterface(InterfaceId_IERC721);
  const isERC1155 = await erc165.supportsInterface(InterfaceId_IERC1155); // check for token approval

  if (isERC721) {
    const asset = new ContractWrapper(signerOrProvider, assetContract, Erc721Abi, {});
    ...
    const asset = new ContractWrapper(signerOrProvider, assetContract, Erc1155Abi, {});
    const approved = await asset.readContract.isApprovedForAll(from, marketplaceAddress);
...
class MarketplaceAuction {
  ...
  async getWinner(listingId) {
    ...
    const endTime = BigNumber.from(listing.endTimeInEpochSeconds); // if we have a winner in the map and the current time is past the endtime of the auction return the address of the winner
...
_defineProperty(Edition, "contractAbi", ABI);
...
_defineProperty(EditionDrop, "contractAbi", ABI$1);
...
_defineProperty(Marketplace, "contractAbi", ABI$2);
...
_defineProperty(Multiwrap, "contractAbi", ABI$3);
...
_defineProperty(NFTCollection, "contractAbi", ABI$4);
...
_defineProperty(NFTDrop, "contractAbi", ABI$5);
...
_defineProperty(Pack, "contractAbi", ABI$6);
...
_defineProperty(SignatureDrop, "contractAbi", ABI$7);
...
_defineProperty(Split, "contractAbi", ABI$8);
...
_defineProperty(Token, "contractAbi", ABI$9);
...
_defineProperty(TokenDrop, "contractAbi", ABI$a);
...
_defineProperty(Vote, "contractAbi", ABI$b);
...
class ContractFactory extends ContractWrapper {
  ...
  async deploy(contractType, contractMetadata) {
    ...
    try {
      ...
      receipt = await this.sendTransaction("deployProxyDeterministic", [encodedType, encodedFunc, ethers.utils.formatBytes32String(blockNumber.toString())]);
    }
...
class ThirdwebSDK extends RPCConnectionHandler {
  ...
  static fromSigner(signer, network) {
    let options = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : {};
    let storage = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : new IpfsStorage();
  ...
  constructor(network) {
    ...
    let storage = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : new IpfsStorage();
    const signerOrProvider = getProviderForNetwork(network);
  ...
  async resolveContractType(contractAddress) {
    ...
    const remoteContractType = ethers.utils.toUtf8String(await contract.contractType()) // eslint-disable-next-line no-control-regex
    .replace(/\x00/g, "");
    invariant(remoteContractType in REMOTE_CONTRACT_TO_CONTRACT_TYPE, "".concat(remoteContractType, " is not a valid contract type, falling back to custom contract"));
...
class ContractDeployer extends RPCConnectionHandler {
  ...
  async deployViaFactory(factoryAddress, implementationAddress, implementationAbi, initializerFunction, initializerArgs) {
    const signer = this.getSigner();
    invariant(signer, "signer is required"); // TODO only require factory interface here - IProxyFactory
  ...
  async deployContractFromUri(publishMetadataUri, constructorParamValues) {
    ...
    if (isDeployableViaFactory && factoryDeploymentData) {
      ...
      invariant(factoryDeploymentData.factoryAddresses, "factoryAddresses is required");
      ...
      invariant(factoryAddress, "factoryAddress not found for chainId '".concat(chainId, "'"));
      invariant(implementationAddress, "implementationAddress not found for chainId '".concat(chainId, "'"));
  ...
  convertParamValues(constructorParamTypes, constructorParamValues) {
    ...
    return constructorParamTypes.map((p, index) => {
      ...
      if (p === "bytes32") {
        invariant(ethers.utils.isHexString(constructorParamValues[index]), "Could not parse bytes32 value. Expected valid hex string but got \"".concat(constructorParamValues[index], "\"."));
        return ethers.utils.hexZeroPad(constructorParamValues[index], 32);
      }

      if (p.startsWith("bytes")) {
        invariant(ethers.utils.isHexString(constructorParamValues[index]), "Could not parse bytes value. Expected valid hex string but got \"".concat(constructorParamValues[index], "\"."));
